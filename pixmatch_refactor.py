...
def ReduceGapFromPageTop(wch_section='main page'): # Esta función se encarga de ajustar el espacio superior en secciones específicas de la página de Streamlit, mediante el uso de estilos de CSS.
...
def Leaderboard(what_to_do): # Esta función se encarga de manejar la creación, actualización y visualización del tablero de líderes basado en la acción especificada.
...
def PressedCheck(vcell): # Esta función evalúa y actualiza el estado de un botón presionado en el juego y dependiendo de la elección del jugador se actualizara el puntaje.
...
def PreNewGame(): # Esta función prepara el estado inicial del juego, reseteando puntajes y celdas, y seleccionando un nuevo conjunto de emojis para el tablero.
    total_cells_per_row_or_col = mystate.GameDetails[2]
...
def ScoreEmoji(): # Esta función devuelve un emoji representativo del puntaje actual del jugador.
...
"""
Función NewGame: Esta función inicia una nueva partida configurando el tablero y actualizando la interfaz de usuario en la barra lateral con detalles del juego.
Esta función también maneja la interacción durante el juego, como la respuesta al presionar botones en el tablero y la actualización del puntaje.
Al final del juego, verifica los puntajes y decide si mostrar globos (victoria) o nieve (derrota), y reinicia el juego si es necesario.
"""
def NewGame():
    ResetBoard()
    total_cells_per_row_or_col = mystate.GameDetails[2]
...
def Main(): # La función main inicia un nuevo juego al presionar el botón correspondiente, ademas es la función central que gestiona el flujo de control principal del juego, reiniciando el juego y preparando el tablero según las elecciones del usuario...
